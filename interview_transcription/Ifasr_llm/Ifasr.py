...
class XfyunAsrClient:
    ...
    def _get_wav_duration_ms(self):
        """
        获取音频时长（毫秒，整数）
        支持格式：WAV（wave模块）、MP3/FLAC/M4A（mutagen库）
        """
        ...
        if ext == '.wav':
            try:
                ...
                raise Exception(f"WAV文件解析失败：{str(e)}，请确认文件为标准WAV格式")
            except Exception as e:
                raise Exception(f"获取WAV音频时长失败：{str(e)}")
        
        # MP3/FLAC/M4A格式：使用mutagen库
        elif ext in ['.mp3', '.flac', '.m4a']:
            if not MUTAGEN_AVAILABLE:
                raise Exception(f"处理{ext}格式需要安装mutagen库：pip install mutagen")
            
            try:
                if ext == '.mp3':
                    audio = MP3(self.audio_file_path)
                elif ext == '.flac':
                    audio = FLAC(self.audio_file_path)
                elif ext == '.m4a':
                ...
                raise Exception(f"获取{ext}音频时长失败：{str(e)}")
        
        else:
            # 不支持的格式
            raise ValueError(f"不支持的音频格式：{ext}，支持的格式：.wav, .mp3, .flac, .m4a")
        ...
        while retry_count < max_retry:
            ...
            if process_status == 4:
                ...
                fail_type_msg = {
                    ...
                    4: "音频内容违规",
                    5: "服务异常或其他错误"
                }.get(fail_type, f"未知错误类型({fail_type})")
                raise Exception(f"转写失败：failType={fail_type} - {fail_type_msg}")
...
if __name__ == "__main__":
    # -------------------------- 请替换为你的真实参数 --------------------------
    XFYUN_APPID = "c0d9df4d"  # 你的讯飞appId
    XFYUN_ACCESS_KEY_ID = "206af4dddefea1c442f33f39c11ad96f"  # 你的accessKeyId
    XFYUN_ACCESS_KEY_SECRET = "ZDdkNDE2NTQxNDhhYzI1ZGMwOTA0ZGJh"  # 你的accessKeySecret
    AUDIO_FILE = "../mp3data/test.MP3"  # 音频文件路径（支持WAV/MP3/M4A/FLAC）
    ...
    try:
        ...
        # 保存完整API响应到JSON文件（用于调试）
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        audio_basename = os.path.splitext(os.path.basename(AUDIO_FILE))[0]
        api_response_file = os.path.join(OUTPUT_DIR, f"{audio_basename}_API响应.json")
        
        with open(api_response_file, 'w', encoding='utf-8') as f:
            json.dump(final_result, f, ensure_ascii=False, indent=2)
        print(f"\n✅ API完整响应已保存到：{api_response_file}")

        # 启用debug模式，显示说话人识别详情
        print("\n" + "="*50)
        print("=== 说话人识别分析 ===")
        result = orderResult.parse_order_result(final_result, with_speaker=True, debug=True)
        print("="*50)
        ...
        # 保存转写结果到文件
        output_file = os.path.join(OUTPUT_DIR, f"{audio_basename}_转写结果.txt")
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"音频文件：{AUDIO_FILE}\n")
            f.write(f"订单ID：{asr_client.order_id}\n")
            f.write(f"音频时长：{asr_client.audio_duration} 毫秒\n")
            f.write("="*50 + "\n")
            f.write("转写文本：\n")
            f.write(result)
